...
<div id="wrapper">
    ...
    $queryNbCocktails = $bdd->query("SELECT COUNT(DISTINCT nom) AS nbCocktails FROM Recettes JOIN Liaison ON Liaison.nomRecette = Recettes.nom WHERE nomIngredient = '" . $_SESSION['categorieCourante'] . "' OR  nomIngredient IN (" . sousCategorie($_SESSION['categorieCourante']) . ")");
    ...
    $queryCocktails = $bdd->query("SELECT DISTINCT nom FROM Recettes JOIN Liaison ON Liaison.nomRecette = Recettes.nom WHERE nomIngredient = '" . $_SESSION['categorieCourante'] . "' OR  nomIngredient IN (" . sousCategorie($_SESSION['categorieCourante']) . ")");
    ...
    for ($i = 0; $i < sizeof($resultQuery2); $i++) {
        $temp = str_replace(")", " ", str_replace("(", " ", str_replace("'", "", $resultQuery2[$i]['nom'])));
            ...
            for ($i = 0; $i < $parPage / 2; ++$i) {
                ...
                if (!empty($cocktails[$i])) {
                    ...
                    <div class="w3-quarter">
                        ...
                        <p>Ingrédients
                            : <?= str_replace("|", ", ", $cocktails[$i]['ingredients']) //On remplace les | dans la description par des , pour rendre la description plus lisible  ?></p>
            ...
            <div class="w3-center w3-padding-32">
                <div class="w3-bar">
                    ...
                    <a href="./?page=<?= $currentPage - 1 ?>"
                       class="w3-bar-item w3-button w3-hover-black <?= ($currentPage == 1) ? "disable" : "" ?>">«</a>
                    ...
                    <?php for ($i = 1; $i <= $nbPages; ++$i) {
                        ?>
                        <a href="./?page=<?= $i ?>"
                           class="<?= ($currentPage == $i) ? "w3-bar-item w3-black w3-button" : "w3-bar-item w3-button w3-hover-black" ?>"><?= $i ?></a>
                    <?php } ?>
                    ...
                    <a href=" ./?page=<?= $currentPage + 1 ?>"
                       class="w3-bar-item w3-button w3-hover-black <?= ($currentPage == $nbPages) ? "disable" : "" ?>">»</a>
                </div>
            </div>
            <hr id="about">
        </div>
